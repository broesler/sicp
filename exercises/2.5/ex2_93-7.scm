...
;; Value is correct, but not reduced to lowest terms.

;; Issue is with reduce-poly... somewhere in rational number creation
;; With reduce-poly:
; Value: (rational (polynomial x (3 1) (0 1)) polynomial x (2 1) (0 1))
; (x^3 + 1) / (x^2 + 1) --> wrong :( should be as above

;; Debugging: steps called in add-rat using reduce-poly
;; mul-poly is called here, works properly
(mul (numer rf) (denom rf)) ; Value: (polynomial x (5 1) (3 1) (2 1) (0 1)) ✓

;; add-poly is called here on (numer rf) (denom rf), works properly 
;; (same as twice the above result)
(add (mul (numer rf) (denom rf))
     (mul (numer rf) (denom rf)))
; Value: (polynomial x (5 2) (3 2) (2 2) (0 2)) ✓

;; mul-poly called again, works properly again
(mul (denom rf) (denom rf)) ; Value: (polynomial x (4 1) (2 2) (0 1)) ✓

;; When make-rat is called on above two chunks (add ...) (mul (denom ...)), we
;; call (reduce ...) on the two terms (the 5th and 4th order polys)
(define n (add (mul (numer rf) (denom rf))
     (mul (numer rf) (denom rf))))
(define d (mul (denom rf) (denom rf)))

;; Confirm gcd by hand:
(define g (greatest-common-divisor n d)) ; Value: (polynomial x (2 1) (0 1)) ✓

(define nt (term-list (contents n)))
(define dt (term-list (contents d)))
(define gt (term-list (contents g)))

(reduce-poly (contents n) (contents d))
...
; ;; (negate x) ; Value: ((polynomial x (1 1)) polynomial x (0 1))  WRONG
; ;; (add x x) ; Value: (rational (polynomial x (1 1)) polynomial x (0 1)) WRONG
; ;; (sub x x) ; FAILS!!!
; ;; (mul x x) ; Value: (rational (polynomial x (2 1)) polynomial x (0 1))
; ;; (div x x) ; Value : (rational (polynomial x (0 1)) polynomial x (0 1))
;
